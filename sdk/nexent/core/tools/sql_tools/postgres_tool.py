...
logger = logging.getLogger("postgres_tool")

import psycopg2  # PostgreSQL driver


class PostgreSqlTool(SqlDatabaseBaseTool):
    """Tool for executing SQL queries on PostgreSQL database."""

    name = "postgres_database"
    description = (
        "Execute SQL queries on PostgreSQL database. "
        "This tool provides a standardized interface for AI agents to query PostgreSQL databases. "
    ...
    description_zh = (
        "在 PostgreSQL 数据库上执行 SQL 查询。该工具为 AI 智能体提供标准化的 PostgreSQL 数据库操作接口。"
        "支持参数绑定和安全控制。"
    ...
    inputs = {
        "sql": {
            ...
            "description": (
                ...
                "'SELECT name, email FROM orders WHERE status = $1 AND amount > $2'"
            ),
            "description_zh": (
    ...
    def __init__(
        self,
        host: str = Field(description="PostgreSQL database host IP or domain"),
        user: str = Field(description="PostgreSQL database username"),
        password: str = Field(description="PostgreSQL database password"),
        database: str = Field(description="PostgreSQL database name"),
        port: int = Field(description="PostgreSQL database port", default=5432),
    ...
    def _convert_params_for_postgres(
        ...
        """Convert standard ? placeholders to PostgreSQL $n format."""
        if not parameters:
    ...
    def _execute_query(
        ...
        sql, parameters = self._convert_params_for_postgres(sql, parameters)
